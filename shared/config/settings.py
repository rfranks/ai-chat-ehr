...
class AzureSettings(BaseSettings):
    ...
    endpoint: Optional[str] = Field(
        default=None, description="Azure endpoint for OpenAI compatible APIs"
    )
    deployment_name: Optional[str] = Field(
        default=None, description="Default Azure OpenAI deployment name"
    )
    api_version: Optional[str] = Field(default=None, description="Azure API version")

    model_config = SettingsConfigDict(
        env_prefix="AZURE_", env_file=".env", extra="ignore"
    )
...
class OpenAISettings(BaseSettings):
    ...
    organization: Optional[str] = Field(
        default=None, description="OpenAI organization identifier"
    )
    project: Optional[str] = Field(
        default=None, description="OpenAI project identifier"
    )
    base_url: Optional[str] = Field(
        default=None, description="Custom OpenAI API base URL override"
    )
...
class AnthropicSettings(BaseSettings):
    ...
    base_url: Optional[str] = Field(
        default=None, description="Custom Anthropic API base URL override"
    )

    model_config = SettingsConfigDict(
        env_prefix="ANTHROPIC_", env_file=".env", extra="ignore"
    )
...
class VertexSettings(BaseSettings):
    ...
    project_id: Optional[str] = Field(
        default=None, description="Google Cloud project identifier"
    )
    location: Optional[str] = Field(
        default=None, description="Regional location for Vertex AI"
    )
    credentials_file: Optional[str] = Field(
        default=None, description="Path to Google credentials JSON file"
    )
    model: Optional[str] = Field(
        default=None, description="Vertex AI model identifier override"
    )

    model_config = SettingsConfigDict(
        env_prefix="VERTEX_", env_file=".env", extra="ignore"
    )
...
class ModelDefaults(BaseSettings):
    ...
    provider: str = Field(
        default="openai", description="Identifier for the default provider"
    )
    name: str = Field(default="gpt-3.5-turbo", description="Default model name")
    temperature: Optional[float] = Field(
        default=None, description="Optional default temperature override"
    )

    model_config = SettingsConfigDict(
        env_prefix="DEFAULT_MODEL_", env_file=".env", extra="ignore"
    )
